...
} from "../../services/LegalPages/privacyPolicy";
...
const PrivacyPolicy = () => {
  ...
  const { data: policies, isLoading, error, refetch } = useGetAllPrivacyPoliciesQuery(access_token);
  ...
  const [togglePrivacyPolicyStatus, { isLoading: isToggling }] = useTogglePrivacyPolicyStatusMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    const existing = getCategoryPolicies(category);
    if (!editingId && existing.length > 0) {
      toast.error("Policies already exist for this category. Please edit instead.");
      return;
    }
    const payload = {
      sentences,
    ...
    try {
      if (editingId) {
        await updatePrivacyPolicy({ id: editingId, data: payload, access_token }).unwrap();
        toast.success("Privacy Policy updated successfully.");
      } else {
        await createPrivacyPolicy({ data: payload, access_token }).unwrap();
      ...
      toast.error(err?.data?.error || "Failed to save privacy policy. Please try again.");
    }
  };

  const handleToggleStatus = async (id) => {
    try {
      await togglePrivacyPolicyStatus({
        id,
        data: { updatedBy: 1 },
        access_token,
      }).unwrap();
  ...
  const handleRemoveSentence = (index) => {
    if (sentences.length <= 1) {
      toast.error("You must have at least one policy.");
      return;
    }
  ...
  const handleCategoryClick = (categoryId) => {
    const existing = getCategoryPolicies(categoryId);
  ...
  const getCategoryPolicies = (categoryId) => {
    return policies?.data?.filter((policy) => policy.category === categoryId) || [];
  };
  ...
  const categories = [
    {
      id: "footer",
      name: "Footer Policy",
      description: "Privacy policies displayed in footer sections",
    },
    {
      id: "partner",
      name: "Partner Policy",
      description: "Privacy policies for partner integrations and collaborations",
    },
    {
      id: "login",
      name: "Login Policy",
      description: "Privacy policies for user authentication and access",
    },
    {
      id: "signup",
      name: "Registration Policy",
      description: "Privacy policies for user registration and account creation",
    },
  ];
  ...
  const MobileCategoryCard = ({ category }) => {
    ...
    const hasPolicies = categoryPolicies[0]?.sentences?.length > 0;
    const policyStatus = hasPolicies ? categoryPolicies[0].status : "No Policies";
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4 shadow-sm">
        <div className="flex justify-between items-start mb-3">
          ...
          {hasPolicies ? (
            <PermissionWrapper section="Privacy Policy" action="edit">
              <label className="relative inline-flex items-center cursor-pointer w-9 h-5"
                onClick={(e) => e.stopPropagation()}
                title={policyStatus === "active" ? "Deactivate" : "Activate"}
              >
                <input
                  type="checkbox"
                  checked={policyStatus === "active"}
                  onChange={() => handleToggleStatus(categoryPolicies[0].id)}
                  disabled={isToggling}
        ...
        <div className="flex justify-between items-center">
          <div className="text-sm text-gray-500">
            {hasPolicies ? `${categoryPolicies[0]?.sentences?.length} policies` : 'No policies configured'}
          </div>
          <button
    ...
    <>
      {showForm ? (
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 flex-shrink-0 ml-4">
                  ...
                  <div className="lg:hidden flex items-center gap-2">
                    <PermissionWrapper section="Privacy Policy" action={editingId ? "edit" : "create"}>
                      <button
            ...
            <PermissionWrapper section="Privacy Policy" action={editingId ? "edit" : "create"}>
              <div className="bg-white border border-gray-200 rounded-lg max-w-full">
                <div className="px-4 lg:px-6 py-4 border-b border-gray-200">
                  <h2 className="text-lg font-semibold text-gray-900">Privacy Policy Content</h2>
                  <p className="text-sm text-gray-500 mt-1">
                    {sentences.length} policy{sentences.length !== 1 ? "s" : ""} configured
                  </p>
                </div>

                <form onSubmit={handleSubmit} id="policyForm" className="p-4 lg:p-6">
                  <DndContext
                    sensors={sensors}
        ...
        <div className="flex flex-col h-screen bg-white overflow-hidden">
                ...
                <div className="flex-1 lg:flex-none lg:min-w-0">
                  <div className="text-center lg:text-left">
                    ...
                    <p className="text-gray-600 mt-0.5 lg:mt-1 text-xs lg:text-base hidden lg:block break-words">
                      Manage privacy policies across different application sections
                    </p>
                  </div>
                </div>
          ...
          {/* Content area with proper scrolling */}
          <div className="flex-1 overflow-y-auto w-full p-4 lg:p-6">
            {isLoading && <AdminLoader message="Loading policies..." />}
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                <div className="flex items-center">
                  ...
                  <span className="text-red-800">Error loading privacy policies. Please try again.</span>
                </div>
              </div>
            )}
                        ...
                        {categories.map((category) => {
                          ...
                          return (
                            <tr key={category.id} className="hover:bg-lightGreen/20 transition-colors">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-900">{categoryPolicies[0]?.sentences?.length}</div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                {hasPolicies ? (
                                  <PermissionWrapper section="Privacy Policy" action="edit">
