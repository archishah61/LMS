...
import { useCheckUserChallengeTaskAnswersMutation } from "../../../services/Challenge/userChallengeTaskAPI"
...
export default function ChallengeQuestQuiz() {
  const { access_token } = getStudentToken()
  const navigate = useNavigate()
  const location = useLocation()
  const { userChallengeTaskId } = location.state
  const getStorageKey = (key) => `quiz_${userChallengeTaskId}_${key}`
  const challengeTask = location.state?.challenge
  const timeLimit = challengeTask?.ChallengeTask?.time_limit
  const MAX_FULLSCREEN_WARNINGS = challengeTask?.ChallengeTask?.no_of_warning >= 0 ? challengeTask?.ChallengeTask?.no_of_warning : 3;
  ...
  const allQuestions = [
    ...(challengeTask?.ChallengeTask.TrueFalseChallenges || []).map((q) => ({ ...q, type: "true-false" })),
    ...(challengeTask?.ChallengeTask.FillInTheBlanksChallenges || []).map((q) => ({ ...q, type: "fill-in-the-blank" })),
    ...(challengeTask?.ChallengeTask.MCQChallenges || []).map((q) => ({ ...q, type: "mcq" })),
  ]

  const showAnswer = Boolean(challengeTask?.ChallengeTask?.show_answer);
  ...
  useEffect(() => {
    if (allQuestions.length > 0) {
      const activeTab = document.getElementById(`question-tab-${currentStep}`)
      if (activeTab) {
  ...
  const [checkChallengeTask] = useCheckUserChallengeTaskAnswersMutation()
  ...
  const { data: quizAttemptsData, isLoading: isLoadingAttempts } = useGetQuizAttemptsQuery(
    { user_challenge_task_id: Number.parseInt(userChallengeTaskId) },
    { skip: !userChallengeTaskId }
  )
  ...
  // Update the enterFullscreen function to be more robust
  const enterFullscreen = useCallback(async () => {
    try {
      ...
      // Clear interval after 3 seconds to prevent memory leaks
      setTimeout(() => clearInterval(checkFullscreenInterval), 3000)

    } catch (error) {
      console.error("Error entering fullscreen:", error)
      throw error // Re-throw to be caught by handleReenterFullscreen
    }
  }, [checkFullscreen])
  ...
  const handleSubmit = useCallback(
    async (isTimeout = false, reason = 'manual') => {
      ...
      try {
        ...
        const checkResponse = await checkChallengeTask({
          data: {
            user_challenge_task_id: Number.parseInt(userChallengeTaskId),
        ...
        const transformedResult = {
          ...
          totalQuestions: checkResponse.totalQuestions,
          totalCorrect: checkResponse.details.filter((r) => r.isCorrect).length,
          totalRewardPoints: checkResponse.totalRewardPoints,
          timeUsed: timeLimit * 60 - timeRemaining,
          showResult: checkResponse.showResult,
          details: checkResponse.details.map((r) => {
            ...
            return {
              ...
              rewardPoints: r.isCorrect ? r.rewardPoints : 0,
            }
          }),
        }

        const currentAttemptNumber = (quizAttemptsData?.attempts?.length || 0) + 1
        const isPassed = checkResponse?.passed || false

        if (isPassed) {
          triggerSuccess(isTimeout ? "Time's up! But you still completed the challenge!" : undefined)
        } else {
          if (isTimeout) {
            triggerFailure("Time's up! You didn't complete the challenge this time.")
          } else {
            triggerFailure("You didn't pass this time. Keep practicing!")
          }
        }

        setQuizResults(transformedResult)
        setQuizCompleted(true)
        if (challengeTask?.ChallengeTask?.reward_points > 0 && isPassed && !challengeTask?.is_completed) {
        ...
        console.error("Failed to submit challenge or record attempt:", error)
        toast.error(error.data?.message || "Failed to submit challenge or record attempt")
        submissionStarted.current = false
        if (!isTimeout) {
          setTimerActive(true)
    ...
    [
      ...
      createQuizAttempt,
      quizAttemptsData,
      challengeTask?.user_challenge_id,
      clearQuizState,
      enterFullscreen,
      exitFullscreen,
    ],
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      // If user exits fullscreen and quiz is not completed
      if (!isCurrentlyFullscreen && !quizCompleted && !submitting && !submissionStarted.current) {
        // Don't count if we're in the process of re-entering
        if (Boolean(challengeTask?.ChallengeTask?.is_warning) && !isReentering) {
          setFullscreenExitCount(prev => prev + 1)

          // After 3 warnings (on 4th exit), auto submit without modal
          if (fullscreenExitCount + 1 > MAX_FULLSCREEN_WARNINGS) {
            ...
            // Show warning modal for first 3 exits
  ...
  }, [checkFullscreen, quizCompleted, submitting, fullscreenExitCount, handleSubmit, isReentering])
  ...
  const saveQuizState = useCallback(() => {
    if (quizCompleted || submitting || !userChallengeTaskId || !isStateRestored) return
  ...
  }, [userAnswers, currentStep, timeRemaining, quizCompleted, submitting, userChallengeTaskId])
  ...
  useEffect(() => {
    let timer
    if (timerActive && timeRemaining > 0 && !quizCompleted && !submissionStarted.current) {
      timer = setInterval(() => {
  ...
  }, [timerActive, timeRemaining, quizCompleted, handleSubmit])
  ...
  useEffect(() => {
    const initializeQuiz = () => {
      ...
      if (savedState) {
        try {
          ...
          if (isStateValid && userChallengeTaskId) {
    ...
    if (userChallengeTaskId && !isStateRestored) {
      initializeQuiz()
    } else {
  ...
  }, [userChallengeTaskId, timeLimit, isStateRestored])
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (Boolean(challengeTask?.ChallengeTask?.is_warning) && document.hidden && !submissionStarted.current && !quizCompleted) {
        setFullscreenExitCount(prev => prev + 1)
  ...
  }, [quizCompleted, submissionStarted.current])
  ...
  const FullscreenWarningModal = () => {
    ...
    // ✅ Removed local isReentering state (we use the outer one)
    const handleReenterClick = async () => {
      if (isReentering) return // Prevent double clicks
      ...
      try {
        ...
        // Modal stays open on failure so user can try again
      } finally {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-2 sm:p-4 z-50">
        <div className="bg-white rounded-lg sm:rounded-xl shadow-xl p-4 sm:p-6 w-full max-w-[280px] xs:max-w-[320px] sm:max-w-md transform transition-all duration-300 scale-100">
          <div className="text-center">
            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-amber-100 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
              <Maximize2 className="w-6 h-6 sm:w-8 sm:h-8 text-amber-600" />
            </div>
            <h3 className={`${textSizes.title} font-bold text-gray-800 mb-1 sm:mb-2`}>
              Fullscreen Mode Required
            </h3>
            <p className={`${textSizes.small} sm:${textSizes.body} text-gray-600 mb-3 sm:mb-4`}>
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className={`w-full py-2 sm:py-3 px-4 bg-forestGreen text-white font-bold rounded-lg transition-colors text-xs sm:text-sm 
                ${isReentering
  ...
  const QuizWarningModal = () => {
    const getModalConfig = () => {
      switch (modalType) {
        case 'timeout':
          return {
            icon: <Clock className="w-6 h-6 sm:w-8 sm:h-8 text-orange-600" />,
            title: "Time's Up!",
        ...
        case 'tab-change':
          return {
            icon: <AlertTriangle className="w-6 h-6 sm:w-8 sm:h-8 text-red-600" />,
        ...
        case 'left-early':
          return {
            icon: <AlertTriangle className="w-6 h-6 sm:w-8 sm:h-8 text-amber-600" />,
        ...
        case 'fullscreen-exceeded':
          return {
            icon: <Maximize2 className="w-6 h-6 sm:w-8 sm:h-8 text-red-600" />,
        ...
        default:
          return {
            icon: <AlertTriangle className="w-6 h-6 sm:w-8 sm:h-8 text-gray-600" />,
            title: "Warning",
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-50">
        <div className="bg-white rounded-lg sm:rounded-xl shadow-xl p-4 sm:p-6 w-full max-w-[280px] xs:max-w-[320px] sm:max-w-md transform transition-all duration-300 scale-100">
          <div className="text-center">
            <div className={`w-12 h-12 sm:w-16 sm:h-16 ${config.bgColor} rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4`}>
              {config.icon}
            </div>
            <h3 className={`${textSizes.title} font-bold text-gray-800 mb-1 sm:mb-2`}>{config.title}</h3>
            <p className={`${textSizes.small} sm:${textSizes.body} text-gray-600 mb-3 sm:mb-4`}>{config.message}</p>

            {config.showSpinner ? (
              <div className="flex items-center justify-center">
                <Loader2 className="w-5 h-5 sm:w-6 sm:h-6 animate-spin mr-2" style={{ color: config.textColor }} />
                <span className={`${config.textColor} ${textSizes.small} sm:${textSizes.body} font-medium`}>{config.buttonText}</span>
              </div>
            ) : config.showActions ? (
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-center mt-3 sm:mt-4">
                <button
                  onClick={handleStayOnPage}
                  className={`${buttonPadding} bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors ${textSizes.small} sm:${textSizes.body}`}
                >
                  Stay on Page
                </button>
                <button
                  onClick={handleLeavePage}
                  className={`${buttonPadding} bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors ${textSizes.small} sm:${textSizes.body}`}
                >
                  Leave Anyway
                </button>
              </div>
  ...
  const getQuestionBadgeColor = (type) => {
    switch (type) {
      ...
      case "true-false": return "bg-indigo-100 text-indigo-800"
      case "fill-in-the-blank": return "bg-pink-100 text-pink-800"
  ...
  if (!challengeTask?.ChallengeTask) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <AlertTriangle className="w-6 h-6 sm:w-8 sm:h-8 text-red-600 mr-2" />
        <span className={textSizes.body}>No challenge task data found</span>
      </div>
    )
  }
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6">
        <div className="bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6">
          <div className={responsiveGrid}>
            ...
            <div className={`${!showAnswer ? 'max-w-6xl mx-auto w-full' : ''} ${sidebarCols} space-y-4 md:space-y-6 lg:sticky lg:top-4`}>
              ...
              <div className={`${!showAnswer ? 'text-center' : 'text-left'} space-y-1 sm:space-y-2`}>
                <h2 className={`${!showAnswer ? 'text-2xl md:text-3xl' : textSizes.title} font-bold text-slate-800 line-clamp-2 leading-tight`}>
                  {challengeTask.ChallengeTask.title}
                </h2>
              ...
              <div className={`${!showAnswer ? 'flex justify-center' : ''}`}>
                {quizResults.totalCorrect === quizResults.totalQuestions ? (
                  <div className={`inline-flex items-center justify-center bg-primary/10 text-forestGreen/85 px-3 py-2 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl border border-primary/20 font-bold ${!showAnswer ? 'text-sm sm:text-base w-auto' : 'text-xs sm:text-sm w-full'}`}>
                  ...
                  <div className={`inline-flex items-center justify-center bg-orange-50 text-forestGreen/85 px-3 py-2 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl border border-orange-200 font-bold ${!showAnswer ? 'text-sm sm:text-base w-auto' : 'text-xs sm:text-sm w-full'}`}>
              ...
              {!showAnswer ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {/* Score Card */}
                  <div className="bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 flex items-center gap-3 sm:gap-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="relative w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 flex-shrink-0">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center text-[10px] sm:text-xs font-bold text-forestGreen">
                        {Math.round((quizResults.totalCorrect / quizResults.totalQuestions) * 100)}%
                      </div>
                    </div>
                    <div>
                      <h3 className="text-sm sm:text-base font-bold text-slate-800">Score</h3>
                  ...
                  {/* Points Card */}
                  <div className="bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 flex items-center gap-3 sm:gap-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 flex items-center justify-center bg-orange-50 rounded-full flex-shrink-0">
                    ...
                    <div>
                      <h3 className="text-sm sm:text-base font-bold text-slate-800">Points</h3>
                      <p className="text-xs sm:text-sm text-slate-500 font-medium mt-0.5">
                  ...
                  {/* Time Card */}
                  <div className="bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 flex items-center gap-3 sm:gap-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 flex items-center justify-center bg-indigo-50 rounded-full flex-shrink-0">
                      <Clock className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-indigo-500" />
                    </div>
                    <div>
                      <h3 className="text-sm sm:text-base font-bold text-slate-800">Time</h3>
                      <p className="text-xs sm:text-sm text-slate-500 font-medium mt-0.5">
                ...
                <div className="space-y-2 sm:space-y-3">
                  {/* Score Card */}
                  <div className="w-full bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 flex items-center gap-3 sm:gap-4 border border-gray-100 shadow-sm">
                    <div className="relative w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 flex-shrink-0">
                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                        <path
                          d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                    ...
                    <div>
                      <h3 className={`${textSizes.body} sm:text-lg font-bold text-slate-800`}>Score Achieved</h3>
                      <p className={`${textSizes.small} text-slate-500 font-medium mt-0.5`}>
                  ...
                  <div className="w-full bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 flex items-center gap-3 sm:gap-4 border border-gray-100 shadow-sm">
                    ...
                    <div>
                      <h3 className={`${textSizes.body} sm:text-lg font-bold text-slate-800`}>Points Earned</h3>
                      <p className={`${textSizes.small} text-slate-500 font-medium mt-0.5`}>
                  ...
                  <div className="w-full bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 flex items-center gap-3 sm:gap-4 border border-gray-100 shadow-sm">
                    ...
                    <div>
                      <h3 className={`${textSizes.body} sm:text-lg font-bold text-slate-800`}>Time Taken</h3>
                      <p className={`${textSizes.small} text-slate-500 font-medium mt-0.5`}>
        ...
        {showBonusPopup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white rounded-lg sm:rounded-xl md:rounded-2xl shadow-2xl w-full max-w-[280px] xs:max-w-[320px] sm:max-w-sm p-4 sm:p-6 md:p-8 text-center">
              <div className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 bg-green-100 text-green-500 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 md:mb-6 shadow-inner">
                <Star className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 fill-current drop-shadow-sm" />
              </div>
              <h3 className="text-lg sm:text-xl md:text-2xl font-black text-slate-800 mb-1 sm:mb-2 tracking-tight">
                Bonus Points!
              </h3>
              <p className={`${textSizes.small} sm:${textSizes.body} text-slate-600 mb-3 sm:mb-4 md:mb-6 font-medium`}>
                Congratulations! This was a <span className="font-bold text-slate-900">Bonus Task</span>.
                You've earned <span className="font-bold text-green-600">+{challengeTask.ChallengeTask.reward_points} extra points</span>!
              </p>
              <button
  ...
  return (
    <div className="container px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8 pb-24 sm:pb-32">
      {!quizCompleted && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          {/* Fullscreen Status Indicator */}
          {Boolean(challengeTask?.ChallengeTask?.is_warning) &&
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 pb-4 sm:pb-5 mb-2 sm:mb-4">
            ...
            <div className="flex flex-col xs:flex-row items-start xs:items-center gap-2 xs:gap-4 w-full xs:w-auto">
              <span className={`${textSizes.small} font-semibold text-slate-500`}>
                Total Questions: {allQuestions.length}
              </span>
              <div className="flex items-center gap-1.5 px-2.5 py-1 sm:px-3 sm:py-1.5 bg-indigo-50 text-indigo-700 rounded-lg border border-indigo-100 font-medium text-xs sm:text-sm">
          ...
          <div className="">
            ...
            <div className="space-y-2 sm:space-y-3 max-w-2xl">
              ...
              {currentQuestion.type === "mcq" && (
                <>
                  {currentQuestion.MCQOptionChallenges
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-40 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-3 sm:py-4 flex justify-between items-center">
              <button
                ...
                onClick={handlePrev}
                className={`${buttonPadding} rounded-lg ${textSizes.small} sm:${textSizes.body} font-medium border transition-all flex items-center gap-1.5 ${currentStep === 0
              ...
              {currentStep < allQuestions.length - 1 ? (
                <button
                  onClick={handleNext}
                  className={`${buttonPadding} bg-slate-900 text-white rounded-lg ${textSizes.small} sm:${textSizes.body} font-medium hover:bg-black transition-all flex items-center gap-2 border border-transparent shadow-sm`}
                >
                  Next
                ...
                <button
                  ...
                  className={`${buttonPadding} bg-primary text-white rounded-lg ${textSizes.small} sm:${textSizes.body} font-medium flex items-center gap-2 shadow-sm ${submitting ? "opacity-75 cursor-not-allowed" : ""}`}
                >
                  {submitting ? (
                    <>
